  ...
  return (
    <div className="relative w-full min-h-0 bg-gray-100 overflow-auto font-sans">
      ...
      {currentStep === 'digit-selection' && (
        <aside
          className="fixed left-0 top-24 z-10 w-24 flex flex-col items-center gap-3 rounded-r-xl border border-l-0 border-grey bg-white py-6 pr-4 pl-3 shadow-sm"
          aria-label="Chiffres disponibles"
        >
          <h3 className="text-darkBlue text-sm font-semibold tracking-wide text-center">
            ...
            {([0, 3, 6, 9] as const).map((digit) => (
              ...
              >
                <button
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-lg border-2 border-grey bg-grey/30 text-darkBlue text-lg font-bold select-none hover:border-blue/60 hover:bg-blue/10 transition-colors"
                  aria-haspopup="true"
      ...
      <div
        className={`relative z-[2] px-4 pt-12 pb-12 sm:px-6 md:px-8 mx-auto ${
          currentStep === 'network-interaction' ? 'max-w-[96vw]' : 'max-w-7xl'
        }`}
      >
