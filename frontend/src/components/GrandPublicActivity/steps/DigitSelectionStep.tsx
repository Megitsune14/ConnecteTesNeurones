...
const DigitSelectionStep = ({ onValidateGrid, initialGrid }: DigitSelectionStepProps) => {
  ...
  return (
    <section className="mx-auto max-w-xl bg-white border-2 border-grey rounded-2xl p-6 shadow-sm animate-fade-in-up overflow-hidden">
                ...
                <div key={rowIndex} className="flex gap-1">
                  {row.map((value, colIndex) => (
                    <button
                      ...
                      className={`w-9 h-9 sm:w-10 sm:h-10 border-2 rounded transition-colors ${
                        value === 1
