...
export function useGrandPublicActivity() {
  ...
  useEffect(() => {
    ...
    const activeOutputs = outputNeurons
      .filter((n) => (n.calculatedOutput ?? 0) > 0)
      .map((n) => ({ digit: n.digit ?? 0, value: n.calculatedOutput ?? 0 }))
    if (activeOutputs.length === 0) {
      setFinalDecision(null)
    } else {
      const winner = activeOutputs.reduce((a, b) =>
        a.value > b.value ? a : b
      )
      setFinalDecision(winner.digit)
    }
  }, [outputNeurons])
